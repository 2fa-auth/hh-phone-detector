...
def write_results_json(path: Path, results):
    path.write_text(
        json.dumps(
...
def write_results_csv(path: Path, results):
...
def write_errors_csv(path: Path, errors):
    with path.open("w", newline="", encoding="utf-8-sig") as file:
...
def main():
    ...
    if args.command == "analyze":
        ...
        analyzer = PhoneAnalyzer(default_region=os.getenv("DEFAULT_PHONE_REGION", "RU"))

        results, errors = analyze_batch(vacancies, analyzer)
        phone_vacancies = sum(result.has_phone for result in results)

        if args.only_with_phone: results = [result for result in results if result.has_phone]
